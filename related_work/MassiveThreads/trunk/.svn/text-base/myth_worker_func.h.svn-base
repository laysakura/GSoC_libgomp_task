...
static inline int myth_increment_index(void)
{
	return __sync_fetch_and_add(&g_thread_index,1);
}
...
static inline void myth_env_init(void)//Initialize
{}
static inline void myth_env_fini(void)//クリーンアップ関数
{}
static inline void myth_set_current_env(myth_running_env_t e)//Set worker thread descriptor
{}
static inline myth_running_env_t myth_get_current_env(void)//Return current worker thread descriptor
...
MYTH_CTX_CALLBACK void myth_startpoint_exit_ex_1(void *arg1,void *arg2,void *arg3)
{
	...
	while (!myth_queue_trypass(&target->runnable_q,th)){
		//失敗したらターゲットを変えて何度でも繰り返す
		target=myth_env_get_randomly();
		th->env=target;
	}
}

//myth_startpoint_init_exで取り込んだコンテキストをpthreadに戻す
static inline void myth_startpoint_exit_ex_body(int rank)
{
	//引数として指定されたrankのワーカースレッドで走っていることを保証する
	myth_running_env_t env;
	//現在の環境を取得
	env=myth_get_current_env();
	//ワーカーの終了フラグを立てる
	myth_notify_workers_exit();
	//startpoint_init_exを呼んだのと同じスレッドで実行されているなら終了
	//本来実行されるべきワーカースレッドのランキューに送り込んでコンテキストスイッチ
	while (env->rank!=rank){
		intptr_t rank_=rank;
		myth_thread_t th;
		th=env->this_thread;
		myth_swap_context_withcall(&th->context,&env->sched.context,myth_startpoint_exit_ex_1,th,rank_,NULL);
		myth_rwbarrier();
