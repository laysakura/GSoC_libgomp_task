...
{//初期化
	...
	for (i=0;i<MYTH_FD_MAP_SIZE;i++){
		...
		myth_fdmap_entry_lock_init(&ret->entry[i].lock);
	}
	//ret->size=0;
	myth_fdmap_lock_init(&ret->lock);
	return ret;
}
...
{//Set fd_map to entry
	...
	myth_fdmap_lock_lock(&fm->lock);
	fe=&fm->entry[MYTH_FD_MAP_KEY(fd)];
	myth_fdmap_entry_lock_lock(&fe->lock);
	for (i=0;i<fe->size;i++){
		if (fe->key[i]==fd){
			fe->data[i]=data;
			myth_fdmap_entry_lock_unlock(&fe->lock);
			myth_fdmap_lock_unlock(&fm->lock);
			return;
		}
	}
	fe->size++;
...
{//Lookup blocked list from fd, return NULL if lookup fails
	int i;
	myth_fd_map_entry_t fe;
	if (!myth_fdmap_lock_trylock(&fm->lock))return NULL;
	fe=&fm->entry[MYTH_FD_MAP_KEY(fd)];
	if (!myth_fdmap_entry_lock_trylock(&fe->lock)){
		myth_fdmap_lock_unlock(&fm->lock);
		return NULL;
	}
	for (i=0;i<fe->size;i++){
...
static inline void myth_fd_map_destroy(myth_fd_map_t fm)
{
	...
	for (i=0;i<MYTH_FD_MAP_SIZE;i++){
		myth_fd_map_entry_t fe;
		fe=&fm->entry[i];
		myth_fdmap_entry_lock_destroy(&fe->lock);
		myth_free(fe->data);
		myth_free(fe->key);
	}
	myth_fdmap_lock_destroy(&fm->lock);
	myth_free(fm->entry);
	myth_free(fm);
}
...
{
	myth_thread_lock_init(&wl->lock);
	wl->count=0;
	wl->size=1;
	wl->io_ops=malloc(sizeof(myth_io_op_t)*1);
}

static inline void myth_io_wait_list_destroy(myth_io_wait_list_t wl)
{
	myth_thread_lock_destroy(&wl->lock);
	free(wl->io_ops);
}
...
{
	myth_thread_lock_lock(&wl->lock);
	if (wl->count==wl->size){
		wl->size*=2;
		wl->io_ops=realloc(wl->io_ops,sizeof(myth_io_op_t)*wl->size);
	}
	wl->io_ops[wl->count]=op;
	wl->count++;
	myth_thread_lock_unlock(&wl->lock);
}
...
static inline void myth_io_fd_list_push(myth_running_env_t env,myth_io_fd_list_t cl,myth_io_struct_perfd_t fd_data)
{
	...
	myth_thread_lock_unlock(&cl->lock);
}
...
{
	myth_io_struct_perfd_t ret;
	if (myth_thread_lock_trylock(&cl->lock)==0)return NULL;
	if (cl->size>0){
		ret=cl->data[cl->size-1];
		cl->size--;
	}
...
static inline void myth_io_fd_list_destroy(myth_running_env_t env,myth_io_fd_list_t cl)
{
	...
	cl->size=0;
	cl->bufsize=0;
	myth_thread_lock_destroy(&cl->lock);
}
