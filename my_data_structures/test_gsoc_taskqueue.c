...
void* parallel_push_pop_take(void* s)
{
  ...
  for (i = 0; i < GSOC_TASKQUEUE_INIT_SIZE * TESTVAL_EXTENDS / 2; ++i) {
    ...
    if (task && data->id == data->logged_worker)
      printf("%d is popped by CPU%d\n", task->threadid, sched_getcpu()); /* These values are evaluated by `make test' script */
  }
    ...
    if (!task)
      {
        ...
        if (task && victim == data->logged_worker)
          printf("%d is taken by CPU%d from CPU%d\n", task->threadid, sched_getcpu(), (int)victim); /* These values are evaluated by `make test' script */
        else
          return NULL;
